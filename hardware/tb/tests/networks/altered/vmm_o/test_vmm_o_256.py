...
@pytest.mark.myhdl
@pytest.mark.altered
class test_vmm_o_256(unittest.TestCase):
  logger = logging.getLogger(__name__)
  logger.setLevel(logging.DEBUG)

  @staticmethod
  def gen_test_params(**kwargs):
      return basic_test_util.gen_test_params(model_name = "vmm_o", altered = True, num_axons = 256, num_neurons = 256, **kwargs)

  def test(self):
    test_params = test_vmm_o_256.gen_test_params(dma_bus_width = 32, dma_frame_header_word_width = 32, router_buffer_depth = 4)
